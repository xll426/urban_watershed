...
class UNet(nn.Module):
    ...
    def __init__(self, num_classes=1,  pretrained=False, use_batchnorm=True, freeze_encoder=False):
        ...
        net.conv1 = torch.nn.Conv2d(3, 64, kernel_size=3, padding=1, bias=False)
        ...
        #attention gates
        self.AG1 = Attention_block(encoder_channels[0], encoder_channels[1], encoder_channels[1])
        self.AG2 = Attention_block(out_channels[0], encoder_channels[2], encoder_channels[2])
        self.AG3 = Attention_block(out_channels[1], encoder_channels[3], encoder_channels[3])
        self.AG4 = Attention_block(out_channels[2], encoder_channels[4], encoder_channels[4])
        ...
        self.fuse = nn.Conv2d(num_classes*3, num_classes, kernel_size=1)
    ...
    def forward(self, x):
        ...
        conv3 = self.AG1(conv4, conv3)
        x1 = self.layer1([conv4, conv3])
        conv2 = self.AG2(x1, conv2)
        x2 = self.layer2([x1, conv2])
        conv1 = self.AG3(x2, conv1)
        x3 = self.layer3([x2, conv1])
        conv0 = self.AG4(x3, conv0)
        x4 = self.layer4([x3, conv0])

        #deep supervison
        size = x4.size()[2:]
        dsv1 = self.dsv1(F.upsample_bilinear(x1, size=size))
        dsv2 = self.dsv2(F.upsample_bilinear(x2, size=size))
        dsv3 = self.dsv3(F.upsample_bilinear(x3, size=size))
        dsv4 = self.dsv4(x4)

        fuse = self.fuse(torch.cat([dsv2, dsv3, dsv4], dim=1))

        if self.training:
            return fuse, dsv2, dsv3, dsv4
        else:
            return fuse
