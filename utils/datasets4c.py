...
class RSDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, input_root, target_root,  mode="train", weight=False, debug = False):
        super().__init__()
        self.input_root = input_root
        self.target_root = target_root
        self.ndsm_root = input_root.replace("train_images","train_ndsms")
        self.weight = weight
        self.mode = mode
        if debug == False:
...
#         return len(self.target)
    ...
    def __getitem__(self, idx):
        ...
        image = np.array(io.imread(os.path.join(self.input_root,self.input_ids[idx])), dtype=np.float32)
        mask = np.array(Image.open(os.path.join(self.target_root,self.target_ids[idx])).convert("L")) / 255
        ndsm = np.load(os.path.join(self.ndsm_root, self.target_ids[idx]).replace(".png", ".npy"))
